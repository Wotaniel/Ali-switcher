...
2) ЕСЛИ режим авто (!isManual):
   guard shouldConvert(последнее слово, isRetroactive: false,
...
e) спелл-чек-блок. Выполняется ЕСЛИ выполнены ВСЕ четыре:
...
- ретро АВТО: идут через спелл-чек (§11 e);
- ретро РУЧНОЙ: обходят спелл-чек (§11 e) ← причина ложного срабатывания 2026-09-11 (§20.1);
...
flowchart TD
    ...
    H2 -- "да" --> S
    H2 -- "нет / РУЧНОЙ" --> F{"не всё-заглавное,<br/>НЕ (встроенное + ручной),<br/>≥ 2 симв., не смешанное?"}
    ...
    F -- "нет: спелл-чек пропущен<br/>(заглавные / встроенное+ручной /<br/>1 символ / смешанное)" --> L
    L --> C
```
...
ретро встроенное обходит спелл-чек (§11 e). Авто-режим в этой ситуации вел бы
