...
```
1) последний сегмент:
   РУЧНОЙ: хвостовые сегменты БЕЗ букв («+», цифры, знаки) сливаются в
   lastGap — триггер конвертации = последнее слово С буквами (PR #37:
   «Rjgbz eljcnjdthtybz + » — символ-хвост раньше убивал весь конверт);
   АВТО: без перестановки, последний сегмент как был.
...
a2) сегмент БЕЗ букв (цифра, «+», знаки) → ПРОЗРАЧЕН: остаётся на месте
   (слово+промежуток переносятся в конверт как есть), обход идёт дальше —
   раскладка к бессмысленному символу неприменима (PR #37: «xthtp 2 ytltkb»
   → «через 2 недели»; раньше цифра стопорила обход)
...
flowchart TD
    ...
    C -- "да" --> T0{"сегмент без букв?<br/>(цифра, «+», знаки)"}
    T0 -- "да: прозрачен,<br/>остаётся на месте" --> C
    T0 -- "нет" --> D{"другой алфавит?"}
    D -- "да" --> S["СТОП"]
