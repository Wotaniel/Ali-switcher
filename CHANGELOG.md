...
### Fixed
- **Trailing symbol killed whole manual conversion** — manual double-Shift with a letter-less tail («Rjgbz eljcnjdthtybz + », user log 2026-09-18 09:33) made «+» the trigger "word"; Translit could not convert it → plan nil → silent layout toggle, three presses did nothing. Now in manual mode the trigger is the last word WITH letters; the symbol tail rides inside lastGap (deleted + retyped as-is). «Rjgbz eljcnjdthtybz + » → «Копия удостоверения + ».
- **Digit in the middle stopped the retro walk** — «xthtp 2 ytltkb» converted only the last word (user log 2026-09-18 12:15): the letter-less «2» was treated as a different-script boundary and the walk never reached «xthtp» («через»). Now letter-less segments are transparent everywhere in the walk (like ChunkFinder): they stay in place, words on both sides convert. «xthtp 2 ytltkb» → «через 2 недели», both modes.
- **Manual retro walk corrupted correct builtin words** — double-Shift after typing correct Russian followed by a wrong-layout word converted the correct builtins too: «смотри,… но все штзгеы» → «yt dct inputs» (2026-09-11 user log; «штзгеы» is «inputs» typed in RU layout). The manual-mode builtin bypass let the spell-checker-free walk convert «но»→«yt», «все»→«dct». Now builtins go through the spell-checker in BOTH modes (auto already did since PR #25): valid Russian words stop the walk unless converted forms are also dictionary-valid (direction tiebreak). «это из сдд» manual now converts «из»→«bp» + «сдд»→«cll» exactly like auto. Regression test added: «но все штзгеы» (manual) → «inputs» only.
...
- **`?` turned into `.` when converting to Russian (Mac layout map)** — the punctuation section of `Translit.ruToEn` used the Windows ЙЦУКЕН placement: `.`↔`/` (key 44) and `?`↔`&` (Shift+7). On the macOS `com.apple.keylayout.Russian` layout, Shift+7 produces `.` and `?` lives on Shift+key 44 (identical in both layouts) — so re-typing `?` after conversion produced a period. The map was rebuilt from a UCKeyTranslate dump of the actual Mac layout: `:`↔`%`, `,`↔`^`, `.`↔`&`, `;`↔`*`, `%`↔`$`, `?`↔`?` (universal), `/`↔`/` (universal). Letters and `№`/`"`/`ё` are unchanged — they already match the Mac layout.
