...
**План:**
- [ ] Единый Logger с уровнями `.debug / .info / .warn / .error`
- [ ] Запись в `~/Library/Logs/AliSwitcher.log` с ротацией (1 МБ, 3 файла)
- [ ] Меню: "Export Debug Log…" → собирает лог + версию + ОС в файл (для баг-репортов)
- [ ] Уровень логирования настраивается в UserDefaults (`debugLevel`)
